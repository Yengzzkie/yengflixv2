...
export default function Home() {
  ...
  return (
    <main>
      ...
      <MovieCarousel>
        <ScrollLeftButton onClick={() => scrollLeft(movieCarouselRef)}>
          ‹
        </ScrollLeftButton>
        <CarouselContainer ref={movieCarouselRef}>
          {data &&
            data.slice(0, 10).map((movie, index) => {
              ...
              const isNew =
                releaseDate >= threeMonthsAgo && releaseDate <= currentDate;
                ...
                <MovieItem key={movie.id}>
                  ...
                  {!isMobile && (
                    <DescriptionPopup className="description">
                      ...
                      <Title>
                        {movie.title} {isNew && <NewBadge>NEW</NewBadge>}
                      </Title>
        ...
        <ScrollRightButton onClick={() => scrollRight(movieCarouselRef)}>
          ›
        </ScrollRightButton>
      </MovieCarousel>
      ...
      <MovieCarousel>
        <ScrollLeftButton onClick={() => scrollLeft(tvCarouselRef)}>
          ‹
        </ScrollLeftButton>
        <CarouselContainer ref={tvCarouselRef}>
          {topTV &&
            topTV.slice(0, 10).map((tv, index) => {
                ...
                <MovieItem key={tv.id}>
                  ...
                  {!isMobile && (
                    <DescriptionPopup className="description">
                      ...
                      <RoundButton onClick={() => handleClick(tv)}>
                        {added ? (
                          <FontAwesomeIcon icon={faCheck} />
                        ) : (
                          <FontAwesomeIcon icon={faPlus} />
                      ...
                      <Title>
                        {tv.name} {isNew && <NewBadge>NEW</NewBadge>}
                      </Title>
        ...
        <ScrollRightButton onClick={() => scrollRight(tvCarouselRef)}>
          ›
        </ScrollRightButton>
      </MovieCarousel>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
        <div>
          <h2 className="carousel-header">Movies you may like</h2>
          <ExpandableCardDemo data={data} className="col-start-1" />
        </div>
        <div>
          <h2 className="carousel-header">TV Series you may like</h2>
          <ExpandableCardDemo data={data} className="col-start-2" />
        </div>
        <div>
          <h2 className="carousel-header">Recommendations</h2>
          <ExpandableCardDemo data={data} className="col-start-3" />
        </div>
      </div>
      ...
      <MovieCarousel>
        <ScrollLeftButton onClick={() => scrollLeft(recentCarouselRef)}>
          ‹
        </ScrollLeftButton>
        <CarouselContainer ref={recentCarouselRef}>
          {recentlyViewed.length === 0 ? (
            <span className="text-center w-full">
              No movie recently viewed yet
            </span>
          ) : (
            recentlyViewed &&
            recentlyViewed.slice(0, 10).map((movie) => {
              return (
        ...
        <ScrollRightButton onClick={() => scrollRight(recentCarouselRef)}>
          ›
        </ScrollRightButton>
      </MovieCarousel>
      ...
      <MovieCarousel>
        <ScrollLeftButton onClick={() => scrollLeft(continueWatchCarouselRef)}>
          ‹
        </ScrollLeftButton>
        <CarouselContainer ref={continueWatchCarouselRef}>
          {continueWatch.length === 0 ? (
            <span className="text-center w-full">No movies watched yet</span>
          ) : (
            continueWatch &&
            continueWatch.slice(0, 10).map((movie) => {
              return (
                <Link key={movie.id} to={`/details/${movie.id}`}>
                  <li className="recently-viewed-item">
                    {isMobile ? (
        ...
        <ScrollRightButton
          onClick={() => scrollRight(continueWatchCarouselRef)}
        >
          ›
        </ScrollRightButton>
      </MovieCarousel>
